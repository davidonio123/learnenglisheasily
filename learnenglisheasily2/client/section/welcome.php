<div class="container text-center mt-4 pt-4 mb-2 pb-2">
    <p class="text mainColor ">Welcome to your private page</p>
</div>
<script src="./client/asserts/js/check_session.js"></script>
<div class="w-100">
    <div class="container">
        <!-- profilo utente -->
        <div class="row" style="border-radius: 12px;height:auto; overflow: hidden; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
            <!-- immagine profilo -->
            <div class="col-md-3 m-0 p-0 d-flex justify-content-center" style="height: 1fr; width: 1fr;">
                <img style="height: 100%; width: 100%;" class="img fluid" src="https://gravatar.com/avatar/697cc154031eb718882903ac1ff4dd2a?d=identicon&s=256" alt="image not found">
            </div>
            <!-- informazioni -->
            <div class="col-md-8 p-4">
                <p class="text mainColor mid">
                    <span id="UserName">Davide</span> <span id="UserSurname">Barretta</span> <span id="UserClass">4BI</span>
                    <span class="badge badge-novizio ml-2">Studente</span>
                    <span class="badge badge-novizio ml-2">Programmatore</span>
                </p>
                <p class="text lightGray down mt-3">
                    Email: <span id="UserEmail">davide.barretta21@iti-marconi.edu.it</span>
                </p>
                <p class="text lightGray mt-3">
                    240 punti
                </p>
                <div class="link">
                    <a href="#">Modifica profilo</a>
                </div>
            </div>
        </div>
        <!-- esercizi svolti -->
        <div class="row mt-3 p-4" style="border-radius: 12px;height:auto; overflow: hidden; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
            <p class="text mainColor mid mb-3">Esercizi svolti</p>
                nessun esercizio svolto fin ora.
            </div>
        </div>
    </div>
</div>
<script src="./client/asserts/js/welcome.js"></script>
<style>
    .badge-novizio {
        background-color: var(--mainColorLight);
        font-size: 0.9rem;
        letter-spacing: normal;
        transform: translateY(-3px);
    }

    .link {
        position: relative;
        margin-top: 130px;
    }

    .badge-success {
        background-color: #28a745;
    }

    .badge-warning {
        background-color: #ffc107;
    }

    .badge-danger {
        background-color: #dc3545;
    }
</style>